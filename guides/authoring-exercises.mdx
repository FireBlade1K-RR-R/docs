...
<Steps>
  ...
  <Step title="Publish the exercise">
    When your metadata, archives, and steps are complete, toggle the exercise to **Active**. On save, ThreatLab runs a **release deadlock check** and a **QRadar log-source preflight** before the exercise goes live — see [Publish-time deadlock check](#publish-time-deadlock-check) and [QRadar log-source preflight](#qradar-log-source-preflight) for what each validates and how to handle warnings.
...
<Tabs>
  ...
  <Tab title="Publish (Active)">
    ...
    - **QRadar log-source preflight** runs against every enabled `syslog_tcp` destination that has **Provision log source** turned on. If QRadar provisioning fails — or a successful provision still needs **Deploy Changes** in the QRadar console — the exercise stays in **Draft** with warnings shown inline. See [QRadar log-source preflight](#qradar-log-source-preflight).
  </Tab>
</Tabs>
...
The preflight **blocks activation** in two cases:
